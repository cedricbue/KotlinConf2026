...
import org.course.challenge01.CurrencyService.Companion.USD
...
class Coroutines01CoroutineContextExerciseTest {
    ...
    fun `Exercise A should call getCurrency(USD) methods of all banks using a threadpool with a single thread`() {
        ...
        val coroutinesMs = measureTimeMillis {
            ...
            runBlocking(context = singleThreadPool) {
                banks.forEach { launch { it.getCurrency(USD) } }
    ...
    @Test
             ...
             *
             * However, there is a twist: you must call the *blocking* [CurrencyService.getCurrencyBlocking] method of the bank's @see [CurrencyService]
             * and NOT the suspending [CurrencyService.getCurrency] method.
             *
             * The challenge is twofold:
             * - you have to ensure that all the calls to [CurrencyService.getCurrencyBlocking] will be *really* executed in parallel.
    ...
    fun `Exercise B should take the lowest rate of all banks executing the blocking call to getCurrencyBlocking(USD) in parallel using async`() {
        val coroutinesMs = measureTimeMillis {
            ...
            runBlocking(context = Dispatchers.IO + MDCContext()) {
                ...
                val minRateOfAllBanks = banks
                    .map { async { it.getCurrencyBlocking(USD) } }
                    .awaitAll()
                    .min()
                ...
                expectedRates.forEachIndexed { idx, rate ->
                    val logMessage = logs[idx].shouldNotBeNull().also {
                        it.message shouldContain "Get currency blocking ${CurrencyService.USD} at rate $rate"
                    }
