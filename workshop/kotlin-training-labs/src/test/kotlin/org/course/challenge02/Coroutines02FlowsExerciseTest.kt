...
class Coroutines02FlowsExerciseTest {
    ...
    fun `Exercise B should create flow generating fibonacci sequence with a delay of 50 ms`(): Unit = runBlocking {
        ...
        val flow: Flow<BigInteger> =
            generateSequence(BigInteger.ZERO to BigInteger.ONE) {
                pair -> pair.second to pair.first + pair.second
            }.map { it.first }.asFlow().onEach { delay(50) }
    ...
    @Test
    @Timeout(5)
    fun `Exercise D should create sharedFlowTriggeredNewsFeedFlow that initially fetches all data via findByIdGreaterThan and later fetches new items by a notification emitted to the MutableSharedFlow`(): Unit =
        runBlocking {
            val dao = NewsFeedDao((1..10).map { NewsItem(it.toLong()) }.toMutableList())
