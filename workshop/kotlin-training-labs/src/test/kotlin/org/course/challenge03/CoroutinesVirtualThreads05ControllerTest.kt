...
import org.course.challenge03.blocking.BlockingJExchangeService
...
import org.course.challenge03.controller_remote.DummyRemoteExchangeController
...
class CoroutinesVirtualThreads05ControllerTest @Autowired constructor(
    ...
    final val registry = loggingMeterRegistry()
     ...
     */
    @Test
    fun `Exercise A should fetch best quote for a stock classic endpoint in parallel`(): Unit {
     ...
     * Do the following:
     * - 1. Implement the [runPerformanceTest] method. Here each parameter is explained:
     ...
     * -- @param [iterationCount]: represents the count of how many times you make calls in parallel to the endpoint above
     * -- @param [startParallelCount]: represents the count of the requests you execute in parallel at the beginning of the test
     * -- @param [maxParallelCount]: represents how many requests will be executed in parallel anytime
     * -- @param [rampUpCount]: represents the amount of parallel calls that will be added to the current parallel count, until maxParallelCount is reached
     * -- @param [callEndpoint]: is the function that calls the actual endpoint.
     *    IMPORTANT: callEndpoint must be wrapped in the measure(...) method, which will ensure we get performance metrics
     */
    suspend fun runPerformanceTest(iterationCount:Int, startParallelCount:Int, maxParallelCount:Int, rampUpCount:Int, callEndpoint:suspend () -> Unit):Unit = coroutineScope {
        TODO("Implement")
    }
